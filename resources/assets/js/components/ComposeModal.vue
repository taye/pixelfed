					...
					<div v-else-if="page == 2" class="w-100 h-100">
						<div v-if="media.length == 1">
                            <template v-if="media[0].type === 'image'">
							    <miru-image-editor-preview :engine="editorEngine" :sourceIndex="0" />
							    <hr>
                                <miru-image-editor-filter-menu :engine="editorEngine" :sourceIndex="0" />
                            </template>
						...
						<div v-else-if="media.length > 1" class="d-flex-inline px-2 pt-2">
							<ul class="nav media-drawer-filters text-center pb-3">
								...
								<li v-for="(m, i) in media" :key="i + ':' + carouselCursor" class="nav-item mx-md-4">
										<div class="nav-link" style="display:block;width:300px;height:300px;" @click="carouselCursor = i">
											...
											<div :class="[m.filter_class?m.filter_class:'']" style="width:100%;height:100%;display:block;">
												<miru-image-editor-preview v-if="m.type === 'image'" :engine="editorEngine" :sourceIndex="i" :class="'rounded ' +  [i == carouselCursor ? ' border border-primary shadow':'']" style="width:100%;height:100%;" />
                                                <img v-else class="d-block img-fluid w-100" src="/storage/no-preview.png" :alt="media[carouselCursor].description" :title="media[carouselCursor].description">
											</div>
										</div>
							...
							<hr>
							<div v-if="media.length > 0 && media[carouselCursor].type == 'image'" class="align-items-center px-2 pt-2">
								<miru-image-editor-filter-menu :engine="editorEngine" :sourceIndex="carouselCursor" />
							</div>
						</div>
						<div v-else>
...
import { MiruImageEditor, createEngine, MiruImageEditorPreview,MiruImageEditorCropper,MiruImageEditorFilterMenu } from '@miru/image-editor/vue2'
...
export default {
    ...
    created() {
        ...
        this.editorEngine = createEngine({
		...
		async mediaUpload() {
			const uploadPromises = this.files.map(async (fileInfo, i) => {
				...
				if (media.type === 'image' && fileInfo.editState) {
					file = await this.editorEngine.exportToBlob(i)
				}
		...
		nextPage() {
			...
			switch(this.page) {
				...
				case 'cropPhoto':
                    ...
                    this.editorEngine.setEditState(this.carouselCursor, croppedState)
					this.page = 2;
				break;
    ...
    watch: {
        files(value) {
			this.editorEngine.setSources(value.map(f => f.file))
		},
	}
}
</script>
